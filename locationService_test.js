...
describe("LocationService", function() {
      ...
      it('sets room', function() {
        LocationService(LocationService().nextRoom("automat"));
        expect($location.url()).toEqual("/r/automat");
      });
      it('sets room and view', function() {
        LocationService(LocationService().nextRoom("automat").nextView("hatch"));
        expect($location.url()).toEqual("/r/automat/v/hatch");
      });
      it('changes view', function() {
          $location.url("/r/automat/v/hatch");
          LocationService(LocationService().nextView("other"); expect($location.url()).toEqual("/r/automat/v/other");
          }); it('returns to room', function() {
          $location.url("/r/room/v/hatch");
          LocationService(LocationService().nextView(null));
